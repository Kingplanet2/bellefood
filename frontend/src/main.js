...
function navigateTo(screenId) {
  ...
  setTimeout(() => current.classList.remove('slide-left'), 400);
...
/* ─────────────────────────────────────────────
   MENU DATA — Default data (overridden by backend)
─────────────────────────────────────────────*/
...
// Live menu data — gets replaced when backend responds
...
async function loadMenuFromBackend() {
  try {
    ...
    if (data && data.main && data.protein && data.drinks) {
      menuData = data;
    }
  } catch (err) {
    // Backend not available — use default menu data
...
function addToCart(name, price, emoji, variant = '', cardEl = null) {
  ...
  else cart[key] = { name: label, price, emoji, qty: 1 };
  updateCartUI();
  bounceCartBadge();
    ...
    if (count === 0) {
      listEl.innerHTML = `
        <div class="cart-empty-screen">
          ...
          <button class="btn-primary" style="margin-top:24px" onclick="navigateTo('menu')">Browse Menu</button>
        </div>`;
...
function updateOrderSummary() {
  ...
  if (items.length === 0) {
    summaryEl.innerHTML = `<div class="checkout-empty-msg"><p>Your cart is empty.<br><strong>Add items from the menu.</strong></p></div>`;
    if (totalRow) totalRow.style.display = 'none';
  } else {
    summaryEl.innerHTML = items.map(item => `
      <div class="order-summary-item">
        <span class="order-item-name">${item.emoji} ${item.name}<span class="order-item-qty">×${item.qty}</span></span>
...
function flyToNav(sourceEl, emoji) {
  ...
  fly.style.cssText = `left:${sourceRect.left + sourceRect.width / 2 - 22}px;top:${sourceRect.top + sourceRect.height / 2 - 22}px;transition:all 0.65s cubic-bezier(0.4,0,0.2,1);`;
...
function buildMenuGrid(category, gridId) {
  ...
  grid.innerHTML = menuData[category].map((item, i) => {
    ...
    return `
      <div class="menu-card" id="${cardId}">
        <div class="card-emoji-wrap" ${hasImage ? 'style="display:none"' : ''}>
          <span class="card-emoji">${item.emoji}</span>
        </div>
        ${hasImage ? `<img class="card-img" src="${item.image}" alt="${item.name}" onerror="this.style.display='none';this.previousElementSibling.style.display='flex'"/>` : ''}
        <div class="card-body">
          ...
          <div class="card-footer">
                ...
                <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">
                  ${item.variants.map(v => `
                    <button class="add-variant-btn"
                      data-name="${item.name}" data-price="${v.price}"
                      data-emoji="${item.emoji}" data-variant="${v.label}"
                      data-card="${cardId}">
                      + ${v.label}
                    </button>
                  `).join('')}
            ...
            ` : `
              ...
              <button class="add-btn"
                ...
                data-emoji="${item.emoji}" data-card="${cardId}">
  ...
  grid.querySelectorAll('.add-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      addToCart(btn.dataset.name, parseInt(btn.dataset.price), btn.dataset.emoji, '', document.getElementById(btn.dataset.card));
    });
  });

  grid.querySelectorAll('.add-variant-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      addToCart(btn.dataset.name, parseInt(btn.dataset.price), btn.dataset.emoji, btn.dataset.variant, document.getElementById(btn.dataset.card));
    });
  });
...
function initPaystack() {
  ...
  const totalKobo = getCartTotal() * 100; // Paystack uses kobo (smallest unit)
  ...
  const handler = PaystackPop.setup({
    ...
    callback: function(response) {
      // Payment successful — now save order to backend
      saveOrderToBackend(response.reference, name, email, phone, addr);
    },
    onClose: function() {
...
─────────────────────────────────────────────*/
async function saveOrderToBackend(paystackRef, name, email, phone, address) {
  const items = Object.values(cart);
  const orderData = {
  ...
  try {
    ...
    saveOrderToLocalHistory(savedOrder);

    // Show success
    showSuccessScreen();

  } catch (err) {
    // Backend unavailable — still show success, save locally
    console.error('Backend error:', err);
    saveOrderToLocalHistory({ ...orderData, _id: paystackRef, createdAt: new Date().toISOString() });
    showSuccessScreen();
  }
}
...
function saveProfile() {
  ...
  if (!name && !phone && !addr) { showToast('⚠️ Please fill in at least one field.'); return; }
  ...
  // Prefill order form
  const fields = [['cust-name',name],['cust-email',email],['cust-phone',phone],['cust-address',addr]];
  fields.forEach(([id, val]) => { const el = document.getElementById(id); if (el && val) el.value = val; });
...
function loadProfile() {
  ...
  const email = localStorage.getItem('bf_email') || '';
  const phone = localStorage.getItem('bf_phone') || '';
  const addr = localStorage.getItem('bf_address') || '';

  const pairs = [
    ['profile-name',name],['profile-email',email],['profile-phone',phone],['profile-address',addr],
    ['cust-name',name],['cust-email',email],['cust-phone',phone],['cust-address',addr],
  ];
  pairs.forEach(([id, val]) => { const el = document.getElementById(id); if (el && val) el.value = val; });
}
...
function saveOrderToLocalHistory(order) {
  ...
  history.unshift({
    id: order._id || order.paystackRef || Date.now(),
    date: new Date(order.createdAt || Date.now()).toLocaleDateString('en-NG', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }),
    items: order.items,
    total: order.total,
...
function renderOrderHistory() {
  ...
  if (!history.length) {
    listEl.innerHTML = `<div class="checkout-empty-msg"><p>No orders yet.<br><strong>Place your first order to see it here.</strong></p></div>`;
    return;
  }
  ...
  listEl.innerHTML = history.map(order => `
    <div class="order-history-item">
      <div class="order-history-header">
        ...
        <span class="order-history-badge">✅ ${order.status || 'Completed'}</span>
      </div>
...
/* ─────────────────────────────────────────────
   CHEF SECTION PARALLAX
─────────────────────────────────────────────*/
function initChefSection() {
  const section = document.getElementById('chef-section');
  const homeScreen = document.getElementById('screen-home');
  const chefImg = document.getElementById('chef-img');
  const parallaxWrap = document.getElementById('chef-parallax');
  if (!section || !homeScreen || !chefImg) return;
  ...
  homeScreen.addEventListener('scroll', () => {
    ...
    if (rel > -window.innerHeight && rel < window.innerHeight) {
      chefImg.style.transform = `scale(1.1) translateY(${rel * 0.2}px)`;
      const progress = Math.max(0, Math.min(1, (scrollY - sectionTop + 400) / 700));
      chefImg.style.filter = `brightness(${0.65 + progress * 0.4})`;
    }
  }, { passive: true });
  ...
  if (window.innerWidth > 768) {
    parallaxWrap.addEventListener('mousemove', (e) => {
      ...
      parallaxWrap.style.transform = `perspective(1200px) rotateX(${y * -5}deg) rotateY(${x * 5}deg)`;
      chefImg.style.transform = `scale(1.1) translate(${x * 10}px, ${y * 10}px)`;
    });
    parallaxWrap.addEventListener('mouseleave', () => {
      parallaxWrap.style.transform = '';
      chefImg.style.transform = 'scale(1.1)';
    });
  }
}
...
function initHeroCanvas() {
  ...
  function draw() {
    ...
    dots.forEach(d => {
      ...
      ctx.fillStyle = d.color; ctx.globalAlpha = 0.5; ctx.fill(); ctx.globalAlpha = 1;
    });
    dots.forEach((a, i) => {
...
function initHeroAnimation() {
  if (window.innerWidth <= 768) return;
  gsap.fromTo('#hero-logo', { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 1.2, ease: 'expo.out', delay: 0.3 });
  gsap.fromTo('#hero-tagline', { opacity: 0 }, { opacity: 1, duration: 1, ease: 'power2.out', delay: 0.7 });
  gsap.fromTo('#hero-cta', { opacity: 0 }, { opacity: 1, duration: 0.8, ease: 'power2.out', delay: 1.0 });
  gsap.fromTo('#hero-stats', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8, ease: 'power2.out', delay: 1.3 });
}
